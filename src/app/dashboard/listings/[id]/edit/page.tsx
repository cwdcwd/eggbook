    ...
    async function loadListing() {
      ...
      try {
        ...
        setFormData({
          ...
          customUnitQty: listing.customUnitQty?.toString() || "",
          stockCount: listing.stockCount?.toString() || "",
        });
        setPhotos(listing.photos || []);
        setSelectedTags(listing.tags?.map((t: { name: string }) => t.name) || []);
        setPricingUnit(listing.unit || "DOZEN");
    ...
    try {
      const res = await fetch(`/api/listings/${listingId}`, {
        ...
        body: JSON.stringify({
          ...
          customUnitQty: formData.customUnitQty ? parseInt(formData.customUnitQty) : null,
          stockCount: formData.stockCount ? parseInt(formData.stockCount) : null,
          photos,
          tags: selectedTags,
          isAvailable,
        }),
  ...
  return (
    <div className="max-w-2xl mx-auto space-y-6">
      <div className="flex items-center gap-4">
        ...
        <h1 className="text-2xl font-bold text-amber-900">Edit Listing</h1>
      </div>
